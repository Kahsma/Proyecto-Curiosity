...
void shell::agregarMovimiento(string tipo,string magnitud,string unidad)
{
  ...
  if(bien == true){ cout <<"El comando de análisis ha sido agregado exitosamente"<<endl;}
...
void shell::agregarElemento(string tipoComp , string tamanio,string unidadMedida,string cordx,string cordy)
{
  ...
  Elemento Ele;
  Ele.setTipoElemento(tipoComp);
  Ele.setTamanno(tamanio);
...
void shell::guardar(queue <string> cInstrucciones, string tipoA, string nomArchivo)
{
  ...
  if(file.is_open()){
    while(!cInstrucciones.empty()){
      file<<cInstrucciones.front()<<endl;
      cInstrucciones.pop();
    }
    file.close();
  }
